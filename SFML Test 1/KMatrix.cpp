...
KMatrix KMatrix::crossprd(KMatrix& rv){
	...
	try{
		...
		//c out << "ERROR: Failed to create Vector3d from KMatrix." << endl;
		KMatrix km(1, 1);
		km(1, 1) = -1;
		return km;
	}
...
void KMatrix::print(int indent, bool spaces){
	...
	for (int r = 0; r < KMatrix::mtrx.rows() ; r++){
		...
		for (int c = 0; c+1 < KMatrix::mtrx.cols() ; c++){
			...
			//c out << KMatrix::mtrx(r, c) << ", ";
		}
		gap = max_abs - (floor(log10(KMatrix::mtrx(r, KMatrix::mtrx.cols()-1))) + 1);
		for (int i = 0 ; i < gap ; i++){
			//c out << ' ';
		}
		//c out << KMatrix::mtrx(r, KMatrix::mtrx.cols()-1) << " |" << endl;
	}
...
KMatrix KMatrix::Range(double begin, double step_size, double end, int rows){
	...
	//c out << "L: " << len << " R: " << rows << " col: " << ncols << endl;
	...
	for (int i = 0 ; i < tokens.size() ; i++){
		if (isnum(tokens[i])){
			...
			if (req_commas && phase != 'c' && phase != 'n'){
				//c out << "ERROR: Failed to create matrix from string - missing comma before token " << i << ", '" << tokens[i] << "'. String:\n\t'" << input << "'" << endl;
				return false;
			}
			...
			if (phase == 'c'){
				if (print_on) //c out << "ERROR: Failed to create matrix from string - semicolon can not follow a comma. String:\n\t'" << input << "'" << endl;
				return false;
			}else if(phase == 'b'){
				if (print_on) //c out << "ERROR: Failed to create matrix from string - semicolon can not be first symbol. String:\n\t'" << input << "'" << endl;
				return false;
			}
			...
			if (c == -1){
				...
				if (print_on) //c out << "ERROR: Failed to create matrix from string:\n\t'" << input << "'" << endl;
				return false;
			}
			phase = 'n'; //new row
			add_line = false;
			...
			if ( r == 0 && ticker == 1){ //begin
				...
				if (print_on) //c out << "ERROR: Failed to create matrix from string due to invalid comma. String:\n\t'" << input << "'" << endl;
				return false;
			}
		}else{
