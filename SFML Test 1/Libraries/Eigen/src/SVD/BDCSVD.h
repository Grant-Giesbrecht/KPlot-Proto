...
void BDCSVD<MatrixType>::divide (Index firstCol, Index lastCol, Index firstRowW, Index firstColW, Index shift)
{
  ...
  //std::c out << "j2 = " << tmp2.transpose().format(bdcsvdfmt) << "\n\n";
  //std::c out << "err:      " << ((tmp1-tmp2).abs()>1e-12*tmp2.abs()).transpose() << "\n";
  static int count = 0;
  //std::c out << "# " << ++count << "\n\n";
...
void BDCSVD<MatrixType>::computeSVDofM(Index firstCol, Index n, MatrixXr& U, VectorType& singVals, MatrixXr& V)
{
...
#ifdef EIGEN_BDCSVD_DEBUG_VERBOSE
  //std::c out << "computeSVDofM using:\n";
  //std::c out << "  z: " << col0.transpose() << "\n";
  //std::c out << "  d: " << diag.transpose() << "\n";
#endif
...
#ifdef EIGEN_BDCSVD_DEBUG_VERBOSE
  //std::c out << "  j:        " << (m_computed.block(firstCol, firstCol, n, n)).jacobiSvd().singularValues().transpose().reverse() << "\n\n";
  //std::c out << "  sing-val: " << singVals.transpose() << "\n";
  //std::c out << "  mu:       " << mus.transpose() << "\n";
  //std::c out << "  shift:    " << shifts.transpose() << "\n";
  
  {
    Index actual_n = n;
    while(actual_n>1 && abs(col0(actual_n-1))<considerZero) --actual_n;
    //std::c out << "\n\n    mus:    " << mus.head(actual_n).transpose() << "\n\n";
    //std::c out << "    check1 (expect0) : " << ((singVals.array()-(shifts+mus)) / singVals.array()).head(actual_n).transpose() << "\n\n";
    //std::c out << "    check2 (>0)      : " << ((singVals.array()-diag) / singVals.array()).head(actual_n).transpose() << "\n\n";
    //std::c out << "    check3 (>0)      : " << ((diag.segment(1,actual_n-1)-singVals.head(actual_n-1).array()) / singVals.head(actual_n-1).array()).transpose() << "\n\n\n";
    //std::c out << "    check4 (>0)      : " << ((singVals.segment(1,actual_n-1)-singVals.head(actual_n-1))).transpose() << "\n\n\n";
  }
#endif
...
#ifdef  EIGEN_BDCSVD_DEBUG_VERBOSE
  //std::c out << "U^T U: " << (U.transpose() * U - MatrixXr(MatrixXr::Identity(U.cols(),U.cols()))).norm() << "\n";
  //std::c out << "V^T V: " << (V.transpose() * V - MatrixXr(MatrixXr::Identity(V.cols(),V.cols()))).norm() << "\n";
#endif
  ...
  //std::c out << "  * j:        " << jsvd.singularValues().transpose() << "\n\n";
  //std::c out << "  * sing-val: " << singVals.transpose() << "\n";
//   //std::c out << "  * err:      " << ((jsvd.singularValues()-singVals)>1e-13*singVals.norm()).transpose() << "\n";
#endif
}
                                         ...
                                         VectorType& singVals, ArrayRef shifts, ArrayRef mus)
{
  ...
  for (Index k = 0; k < n; ++k)
  {
...
#ifdef EIGEN_BDCSVD_DEBUG_VERBOSE
    //std::c out << right-left << "\n";
    //std::c out << "fMid = " << fMid << " " << secularEq(mid-left, col0, diag, perm, diag-left, left) << " " << secularEq(mid-right, col0, diag, perm, diag-right, right)   << "\n";
    //std::c out << "     = " << secularEq(0.1*(left+right), col0, diag, perm, diag, 0)
    ...
    {
#ifdef  EIGEN_BDCSVD_DEBUG_VERBOSE
      //std::c out << "useBisection for k = " << k << ", actual_n = " << actual_n << "\n";
#endif
      ...
      if(!(fLeft * fRight<0))
      {
        //std::c out << "fLeft: " << leftShifted << " - " << diagShifted.head(10).transpose()  << "\n ; " << bool(left==shift) << " " << (left-shift) << "\n";
        //std::c out << k << " : " <<  fLeft << " * " << fRight << " == " << fLeft * fRight << "  ;  " << left << " - " << right << " -> " <<  leftShifted << " " << rightShifted << "   shift=" << shift << "\n";
      }
#endif
      ...
      for(Index l = 0; l<m; ++l)
      {
        ...
        if(i!=k)
        {
          ...
          if(i!=k && std::abs(((singVals(j)+dk)*(mus(j)+(shifts(j)-dk)))/((diag(i)+dk)*(diag(i)-dk)) - 1) > 0.9 )
            //std::c out << "     " << ((singVals(j)+dk)*(mus(j)+(shifts(j)-dk)))/((diag(i)+dk)*(diag(i)-dk)) << " == (" << (singVals(j)+dk) << " * " << (mus(j)+(shifts(j)-dk))
                       << ") / (" << (diag(i)+dk) << " * " << (diag(i)-dk) << ")\n";
#endif
        }
      }
#ifdef EIGEN_BDCSVD_DEBUG_VERBOSE
      //std::c out << "zhat(" << k << ") =  sqrt( " << prod << ")  ;  " << (singVals(last) + dk) << " * " << mus(last) + shifts(last) << " - " << dk << "\n";
#endif
...
void BDCSVD<MatrixType>::deflation(Index firstCol, Index lastCol, Index k, Index firstRowW, Index firstColW, Index shift)
{
  ...
  { 
#ifdef  EIGEN_BDCSVD_DEBUG_VERBOSE
    //std::c out << "deflation 4.1, because " << diag(0) << " < " << epsilon_coarse << "\n";
#endif
  ...
  for (Index i=1;i<length;++i)
    ...
    {
#ifdef  EIGEN_BDCSVD_DEBUG_VERBOSE
      //std::c out << "deflation 4.2, set z(" << i << ") to zero because " << abs(col0(i)) << " < " << epsilon_strict << "  (diag(" << i << ")=" << diag(i) << ")\n";
#endif
      col0(i) = 0;
    }
  ...
  for (Index i=1;i<length; i++)
    ...
    {
#ifdef  EIGEN_BDCSVD_DEBUG_VERBOSE
      //std::c out << "deflation 4.3, cancel z(" << i << ")=" << col0(i) << " because diag(" << i << ")=" << diag(i) << " < " << epsilon_coarse << "\n";
#endif
...
#endif
#ifdef EIGEN_BDCSVD_DEBUG_VERBOSE
  //std::c out << "to be sorted: " << diag.transpose() << "\n\n";
#endif
  {
...
#ifdef EIGEN_BDCSVD_DEBUG_VERBOSE
  //std::c out << "sorted: " << diag.transpose().format(bdcsvdfmt) << "\n";
  //std::c out << "      : " << col0.transpose() << "\n\n";
#endif
  ...
  //condition 4.4
  {
    ...
    for(; i>1;--i)
      ...
      {
#ifdef EIGEN_BDCSVD_DEBUG_VERBOSE
        //std::c out << "deflation 4.4 with i = " << i << " because " << (diag(i) - diag(i-1)) << " < " << NumTraits<RealScalar>::epsilon()*diag(i) << "\n";
#endif
